...
10    CK(1:p%NDIM,ID)=CD(1:p%NDIM,LNDB(ID,IE))
     ...
     !&                    NGR,NGL,GPR,GWR,GPL,GWL,TOLGP,BETA,INT_ELEM)
      ELSE     ! EVALUATE INTEGRAL OVER SINGULAR ELEMENT
       CALL SINGULAR_ELEM(e,p,TOLGP,NDSID,NSEL(IE),NPW,V1E)
!       print *,NSEL
!       print *,XIS
      ENDIF
      VINT=VINT+V1E
20    CONTINUE
      ...
      SUBROUTINE init_hs_elem(p,TOLGP,VINT)
      ...
      subroutine singular_elem(e,p,tolgp,ndsid,nsp,npw,v1e)
          use matrix_mod
          use param_mod
          implicit none
          type(params) :: p
          ...
          integer :: KSB(4)
          real(8) ::  XIQ(p%NBDM),CSUB(2,2),sf0(p%NODE), COEFG(0:p%NPOWG),COEFH(0:NPW),XI0(p%NBDM),V1E(p%NF),   &
              &          RINT(p%NF)
              ...
              DO 80 ISID=1,e%NSUB
                  ...
                  DO ISUB=1,500
                      ...
                      DO IGL=1,iabs(p%NGL)
                          ...
                          CALL COEFS_GH(e,p,XIQ,NPW,COEFG,COEFH)
                          CALL INT_RHO(e,p,NPW,XIQ,sf0,COEFG,COEFH,RINT)
      ...
      SUBROUTINE INT_RHO(e,p,NPW,XIQ,SHAP,COEFG,COEFH,RINT)      
          use param_mod
          implicit none
      type(params) :: p
      type(Elem) ::e
     ...
     &          COEFB(0:11,p%NF),XI(p%NBDM),XQ(p%NDIM),RINT(p%NF),FQ(p%NF)
      integer :: npowf,beta1,k,npw,pw,nbeta
      real(8) :: rhoq
      SLOP=XIQ-p%XIP
...
!       print *,"pw = ",pw
  40  RINT=RINT-(1.D0/RHOQ**PW-COEFH(INT(PW)))/PW*COEFB(K,:)
      ...
      SUBROUTINE COEFS_GH(e,p,XIQ,NPW,COEFG,COEFH)
      use param_mod
      implicit none
      type(params) :: p 
      type(Elem) :: e 
      real(8) :: XIQ(p%NBDM),COEFG(0:p%NPOWG),COEFC(0:NPW),COEFH(0:NPW)
      integer :: n,i,npw

      ! DETERMINE COEFFICIENTS Gn
      COEFG=0.D0
      CALL COEF_G(e,p,XIQ,COEFG)
      ...
      DO N=1,NPW
       ...
       ENDDO
       IF(N.LE.p%NPOWG)COEFC(N)=COEFG(N)+COEFC(N)   ! Eq. (3-6-37)
     ...
     &        -COEFC(1)**4
      ...
      SUBROUTINE COEF_G(e,p,XIQ,COEFG)
     ...
     &          SHAP(p%NODE),RMAT(p%NPOWG,p%NPOWG),COEFG(0:p%NPOWG)
      ...
      IF(p%NODE.EQ.2) THEN         ! Eq.(24)
       COEFG(0)=0.25*((e%CK(1,1)-e%CK(1,2))**2+(e%CK(2,1)-e%CK(2,2))**2)  
     ...
     &         +XBAR2*p%XIP(1)*p%XIP(1)
      ...
      ELSE      ! QUADRILATERAL ELEMENTS
       ...
       GOTO 20
 10    COEFG(IP)=((R2/RHO)**2-COEFG(0))/RHO     ! Eq.(3-6-60)
 ...
 20    CONTINUE
      ...
      SUBROUTINE COEF_B(e,p,NPOWF,      &
     &                  XIQ,SLOP,RHOQ,RI,COSN,GCD,COEFG,COEFB)
        use param_mod
      IMPLICIT REAL*8 (A-H,O-Z)
      type(params) ::p
      type(Elem) ::e
     ...
     &          GCD(p%NDIM,*),XIQ(p%NBDM),XI(p%NBDM),SLOP(p%NBDM),    &
     &          RI(p%NDIM),X(p%NDIM),RMAT(NPOWF,NPOWF),COEFG(0:p%NPOWG),      &
     &          COEFB(0:11,p%NF),SHAP(p%NODE),FQ(p%NF),A(p%NDIM)
      VK=RHOQ/DBLE(NPOWF)
      DO 20 IP=0,NPOWF
      RHO=VK*DBLE(IP)
      XI=p%XIP+RHO*SLOP
      ROBAR=COEFG(0); DO M=1,p%NPOWG; ROBAR=ROBAR+COEFG(M)*RHO**M; ENDDO
