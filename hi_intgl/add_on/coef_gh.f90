    ...
    subroutine compute_coeff_GH(this,xiq)

        class(hs_elem) :: this
        real(rk) :: xiq(this%ndim-1)

        integer,pointer :: npw,npowg
        real(8),pointer :: coefg(:),coefh(:)

        real(8) ::cosn(ndim),gcd(ndim,nbdm),xi(ndim),ri(ndim),shap(node),     &
        &         coefc(0:npw)

        integer :: i,n,ip,jp,nbdm


        npw=>this%params(1)%npw
        npowg=>this%param(1)%n_pwr_g
        coefh=>this%param(1)%coefh
        coefc=>this%param(1)%coefc
        
        !call compute_coeff_g(ndim,nbdm,node,npowg,xp,xip,xiq,    &
        !&           coefg)
        call this%compute_coeff_g(xiq)
        ! determine coefficients cn using eq. (3-6-37)

        coefc(0)=dsqrt(coefg(0))
        do n=1,this%params(1)%npw
            coefc(n)=0.d0
            do i=1,n-1
...
!        COEFG(2)=0.25*XBAR2
            ...
            xip = this%src2D%getArray()

            slop = xiq%getArray() - this%src2D%getArray()
            rhoq = norm2(slop) ! length
            slop=slop/rhoq  ! normalized distance vector
